...
def average_state_results(results_directory, results_file_name, ):
    ...
    for time_step in sorted(accumulated.keys()):
        ...
        binary_average_vectors = np.sign(summed_vectors)
        print(f"{summed_vectors} -> {binary_average_vectors}")
        avg_results.append([time_step, binary_average_vectors.tolist()])
...
#             return -1
#
#     states = [[i[0], [ for _ in range(len(i[1]))]] for i in averaged_vls]
